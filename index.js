...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬ ⏳ ❭  Aguarde...',
				success: '️❬ ✔️ ❭  Comando concluído com sucesso!',
				error: {
			...
			switch(command) {
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('❬ 💎 ❭  Este comando só pode ser usado pelo proprietário do Bot!')
...
case 'lokgi':
                ...
                case 'reportarbug':
                    ...
                    reply('❬ ✔️ ❭ Bug reportado com sucesso, se for mentira será cobrado(a)!🙄')
                    break
					case 'membros2':
					members_id = []
				...
				case 'ytbaixa':
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('❬ ❗ ❭  Digite o link do video do Youtube que deseja baixar!')
			...
			case 'mapa': 
				...
				case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(300,iw)':min'(300,ih)':force_original_aspect_ratio=decrease,fps=15, pad=300:300:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
							.toFormat('webp')
							.save(ran)
					} else {
				...
				case 'tts':
				case 'audio':
					if (args.length < 1) return client.sendMessage(from, '❬ ❗ ❭  Informe o idioma que deseja que eu fale!  exemplo:  pt, es, it, ja...', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '❬ ❗ ❭  Informe o texto que deseja que eu fale!', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/eQskNdS.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Use com moderação!*\n\nxvideos.com'})
					break
					case 'mia':
					if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝙍𝙚𝙖𝙡 𝙂𝙖𝙣𝙜𝙨𝙩𝙚𝙧\n\ninstagram.com/herberthsfc'})
					break
				case 'prefixo':
				case 'sigla':
				...
				case 'adicionar':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'abrir') {
					    reply(`❬ ✔️ ❭ Grupo aberto para todos os membros com sucesso!`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(`❬ ❗ ❭ Grupo fechado para membros comuns com sucesso!`)
       ...
       client.sendMessage(from, '❬ ❗ ❭  Contato do meu criador! Por favor salve!',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
           case 'nomegp':
                if (!isGroup) return reply(mess.only.group)
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'boasvindas':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('❬ ✔️ ❭  Comando concluído com sucesso!')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ ✔️ ❭  Comando concluído com sucesso!')
					} else if (Number(args[0]) === 0) {
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('❬ 💎 ❭  Este comando só pode ser usado pelo proprietário do Bot!') 
					if (args.length < 1) return reply('❬ ✔️ ❭')
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('✔️')
					...
					try {
						...
						mentions(`❬ ✔️ ❭ Foto do perfil atualizada com sucesso! Usando a foto de @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
